  ...
  return (
    <>
      <li className={`${className}`}>
        ...
        <div
          className="mt-3 pl-2 transition-[height] overflow-auto border-l-2 dark:border-gray-600"
          style={{
            height,
            overflow,
        ...
        >
          <div ref={contentRef} className="pl-3 pb-2">
            <h3 className="text-xs text-slate-500 dark:text-slate-300/90  font-medium mb-2">
              Selecciona un puerto
            </h3>
            <div className="pl-1 mt-1 w-full text-xs flex items-center gap-3">
              <Menu
                as="div"
                className="relative inline-block text-left flex-grow"
              >
                {({ open }) => {
                  if (open) {
                                  ...
                                  {({ active }) => (
                                    <div
                                      className={`${active
                                          ...
                                          : "text-gray-700"
                                        } block px-4 py-2 font-semibold text-sm text-slate-500 dark:text-slate-300 cursor-pointer`}
              ...
              <button
                onClick={handleTogglePortBtn}
                className={`${activeBtn ? "bg-indigo-600" : "bg-gray-400 dark:bg-gray-500"
                  } w-11 h-6 px-0.5 flex items-center rounded-full`}
              >
                <span
                  className="w-5 h-5 bg-white rounded-full transition-transform flex items-center justify-center"
                  style={{ transform: `translate(${activeBtn ? 20 : 0}px,0)` }}
                ></span>
              </button>
            </div>
            <h3 className="text-xs text-slate-500 dark:text-slate-300/90 font-medium my-2">
              Estatus del puerto
            </h3>
